...
export async function addGoal(goal: {
  ...
  status: GoalStatus; target_date?: string | null;
}): Promise<Goal | null> {
  try {
...
export async function updateGoal(id: number, updates: Partial<{
...
export default function GoalsSystemPage() {
  ...
  useEffect(() => { fetchMeClient().then(setMe); }, []);
  useEffect(() => {
    if (!me?.id) return;
  ...
  async function handleAddGoal(e: FormEvent) {
    ...
    const created = await addGoal({
      user_id: me.id, title: title.trim(),
