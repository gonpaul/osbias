...
export default function FrameworkListClient({ initialFrameworks }: Props) {
  const t = useTranslations('Frameworks');
  const [frameworks, setFrameworks] = useState<Framework[]>(initialFrameworks);
  const [filteredFrameworks, setFilteredFrameworks] = useState<Framework[]>(initialFrameworks);
  ...
  const allConcepts = Array.from(new Set(frameworks.flatMap(f => f.concepts))).sort();
  ...
  useEffect(() => {
    ...
    if (selectedConcept) {
      filtered = filtered.filter(framework => framework.concepts.includes(selectedConcept));
    }
    if (showSystemOnly) {
  ...
  const handleUseFramework = async (framework: Framework) => {
    try {
      await fetch(`/api/frameworks/${framework.id}/use`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ notes: `Applied framework: ${framework.name}`, completed: false }),
      });
  ...
  const handleCreateFramework = () => setShowCreateModal(true);
  ...
  const handleDeleteFramework = async (framework: Framework) => {
    if (!confirm(t('confirmDelete', { name: framework.name }))) return;
    try {
      const response = await fetch(`/api/frameworks/${framework.id}`, { method: 'DELETE', credentials: 'include' });
      if (response.ok) {
        const updated = frameworks.filter(f => f.id !== framework.id);
        setFrameworks(updated);
        setFilteredFrameworks(updated);
      } else {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const frameworksWithStats = await Promise.all(
            data.map(async (fw: Framework) => {
              try {
                const statsResponse = await fetch(`/api/frameworks/${fw.id}/stats`, { credentials: 'include' });
                if (statsResponse.ok) {
                  const stats = await statsResponse.json();
                  return { ...fw, usage_stats: stats };
                }
              } catch (error) {
                console.error(`Error fetching stats for framework ${fw.id}:`, error);
              }
              return fw;
            })
          );
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <div className="bg-(--darkelbg) rounded-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            >
              <option value="">{t('allConcepts')}</option>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-(--foreground)">{t('systemOnly')}</span>
            </label>
            <div className="flex items-center text-(--secondary)">
              <span>{filteredFrameworks.length} {t('found')}</span>
            </div>
          </div>
        </div>
              ...
              <div className="p-6 flex flex-col h-full">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {framework.concepts.slice(0, 3).map((concept, index) => (
                    <span key={index} className="px-2 py-1 text-xs bg-(--emphasis)/20 text-(--emphasis-light) rounded-full">
                      {concept}
                    </span>
                  ))}
                  {framework.concepts.length > 3 && (
                    <span className="px-2 py-1 text-xs bg-(--secondary)/20 text-(--secondary) rounded-full">
                      {t('more', { count: framework.concepts.length - 3 })}
                    </span>
                  )}
                </div>
                ...
                {framework.usage_stats && (
                  <div className="flex flex-wrap justify-between items-center gap-6 mb-4 text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-(--secondary)">{t('uses')}</span>
                      <span className="text-(--foreground) font-medium">{framework.usage_stats.total_uses}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-(--secondary)">{t('rating')}</span>
        ...
        {filteredFrameworks.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-(--foreground) mb-2">{t('noFrameworks')}</h3>
          </div>
        )}
      </div>
          ...
          <div className="bg-(--darkelbg) rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6 relative">
                <div className="flex-1">
                  <h2 className="text-2xl font-bold text-(--foreground) mb-2">{selectedFramework.name}</h2>
              ...
              <p className="text-(--secondary) mb-6 text-lg">{selectedFramework.description}</p>

              {selectedFramework.concepts.length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedFramework.concepts.map((concept, index) => (
                      <span key={index} className="px-3 py-1 text-sm bg-(--emphasis)/20 text-(--emphasis-light) rounded-full">
                        {concept}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-4">
                    {selectedFramework.steps
                      .sort((a, b) => a.step_order - b.step_order)
                      .map((step) => (
                        <div key={step.id} className="bg-(--background) rounded-lg p-4 border border-(--secondary)/20 hover:border-(--golden)/30 transition-colors">
                          <div className="flex items-start gap-4">
                            <span className="text-(--golden) font-bold text-lg mt-1">{step.step_order}</span>
                            <div>
                              <h4 className="font-semibold text-(--foreground) mb-1">{step.title}</h4>
                              <p className="text-(--secondary) text-sm">{step.description || 'No description provided.'}</p>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    handleUseFramework(selectedFramework);
